...
def train_reg_inv_label(config, basedir):
    ...
    for epoch in range(start_epoch, config["TrainConfig"]["epoch"] + 1):
        ...
        if mean_train_loss_dict.get("inv_label_loss", None) is not None:
            weight_inv_label_loss = mean_train_loss_dict["inv_label_loss"] * config['LossConfig']["inv_label_loss"][
                'weight']
...
def compute_inv_label_loss(label, dvf, inv_dvf, STN_bilinear, loss_function_dict):
    num_classes = torch.max(label).item()
    inv_loss = 0.
    for i in range(1, num_classes + 1):
        label_i = (label == i).float()
        if torch.any(label_i):
            warped_label_i = STN_bilinear(label_i, dvf)
            inv_label_i = STN_bilinear(warped_label_i, inv_dvf)
            inv_loss = inv_loss + loss_function_dict['inv_label_loss'](inv_label_i, label_i)
    return inv_loss
...
def get_loss_function(config):
    ...
    if config['LossConfig']['inv_label_loss']['use']:
        loss_function_dict['inv_label_loss'] = getattr(loss,
                                                       config['LossConfig']['inv_label_loss']['type'])()
