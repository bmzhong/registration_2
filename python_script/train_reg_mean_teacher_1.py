...
def train_reg_mean_teacher_1(config, basedir):
    print("train function: train_reg_mean_teacher_1")
    ...
    for epoch in range(start_epoch, config["TrainConfig"]["epoch"] + 1):
        ...
        for id1, volume1, label1, id2, volume2, label2 in tqdm(train_dataloader):
            ...
            if use_mean_teacher and (label1 == [] or label2 == []):
                noise1 = torch.clamp(torch.randn_like(volume1) * 0.1, -0.2, 0.2).to(device)
                noise2 = torch.clamp(torch.randn_like(volume2) * 0.1, -0.2, 0.2).to(device)

                ema_volume1 = volume1 + noise1
                ema_volume2 = volume2 + noise2
                ...
                if epoch < start_ema_epoch:
                    ...
                    if consistency_loss_type == 1:
                        ...
                        eam_warp_volume1 = STN_bilinear(volume1, ema_dvf)
                        consistency_loss = torch.mean((warp_volume1 - eam_warp_volume1) ** 2)
                    elif consistency_loss_type == 2:
                        consistency_loss = torch.mean((dvf - ema_dvf) ** 2)
                    else:
                        raise Exception()
