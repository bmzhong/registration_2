...
def window_area_partition(x, window_size):
    ...
    windows = x.permute(0, 1, 3, 5, 2, 4, 6, 7).contiguous().view(
        B, d, h, w, reduce(mul, window_size), C)
...
class CrossWindowAttention3D(nn.Module):
    ...
    def forward(self, x, xa):
        ...
        kv = self.kv(xa).reshape(B_, M, 2, self.num_heads, C //
...
class CrossTransformerBlock3D(nn.Module):
    ...
    def forward_part1(self, x, xa):
        ...
        attn_windows = self.cross_attn(
            x_windows, x_area_windows)  # B*nW, Wd*Wh*Ww, C
        # merge windows
        attn_windows = attn_windows.view(-1, *(window_size + (C,)))
        x = window_reverse(attn_windows, window_size, B,
                           Dp, Hp, Wp)  # B D' H' W' C
...
class PatchEmbed3D(nn.Module):
    ...
    Args:
        patch_size (int | tuple): Patch token size. Default: (2,4,4).
...
class XMorpher(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in reversed(range(self.num_layers)):
            concat_linear = nn.Linear(
                2 * int(embed_dim * 2 ** i_layer), int(embed_dim * 2 ** i_layer))
        ...
        self.reverse_patch_embedding1 = nn.ConvTranspose3d(num_classes, num_classes, kernel_size=patch_size,
                                                           stride=patch_size)
        self.reverse_patch_embedding2 = nn.ConvTranspose3d(num_classes, num_classes, kernel_size=patch_size,
                                                           stride=patch_size)
    ...
    def forward(self, moving, fixed):
        ...
        for inx, layer_up in enumerate(self.up_layers):
            if inx == 0:
                ...
                if moving.shape != features_moving[3 - inx].shape:
                    ...
                    moving = F.interpolate(moving, size=(
                        D, W, H), mode='trilinear', align_corners=True)
                    fixed = F.interpolate(fixed, size=(
                        D, W, H), mode='trilinear', align_corners=True)
                    moving = rearrange(moving, 'n c d h w -> n d h w c')
                    fixed = rearrange(fixed, 'n c d h w -> n d h w c')
        ...
        moving = self.norm2(moving)
        fixed = self.norm2(fixed)

        moving = rearrange(moving, 'n d h w c -> n c d h w')
        fixed = rearrange(fixed, 'n d h w c -> n c d h w')

        # reverse the patch embedding to transfer the final feature into image size
        moving = self.conv_num_classes1(moving)
        fixed = self.conv_num_classes2(fixed)

        moving = self.reverse_patch_embedding1(moving)
        fixed = self.reverse_patch_embedding2(fixed)

        if rD % self.patch_size[0] != 0 or rH % self.patch_size[1] != 0 or rW % self.patch_size[2] != 0:
            moving = moving[:, :, :rD, :rH, :rW].contiguous()
            fixed = fixed[:, :, :rD, :rH, :rW].contiguous()

        return moving, fixed


class XMorpherSeg(nn.Module):
    def __init__(self, num_classes, n_channels=1, patch_size=(4, 4, 4), scale=1):
        super().__init__()
        embed_dim = 48
        self.scale = scale
        self.patch_embed = PatchEmbed3D(
            patch_size=(self.scale, self.scale, self.scale), in_chans=n_channels, embed_dim=embed_dim // 2)

        self.swin = XMorpher(num_classes=num_classes, window_size=(
            2, 2, 2), in_chans=n_channels if self.scale == 1 else embed_dim // 2, embed_dim=embed_dim,
                             patch_size=patch_size)

        self.transpose3d = nn.ConvTranspose3d(embed_dim // 2, embed_dim // 2, kernel_size=(2, 2, 2),
                                              stride=(2, 2, 2))
    ...
    def forward(self, moving, fixed):
        if self.scale > 1:
            moving = self.patch_embed(moving)
            fixed = self.patch_embed(fixed)
        moving, fixed = self.swin(moving, fixed)
        if self.scale > 1:
